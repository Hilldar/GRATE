...
while getopts "heDAE" OPTION; do
	...
	D)
	    echo " Decode the RUN"
	    mkdir -p "$TER/data/raw_root/$run_number_opt"
	...
	A)
	    ...
	    bash -c "$exe_ter -C $run_number_opt"
...
if [    $run_number_no_opt ]; then cd; bash -c "source WenGraal.sh ${run_number}"; cd $QUI; exit; fi
